# PRD: WSOPTV - Private Poker VOD Streaming Platform

**Version**: 1.5.0
**Date**: 2025-12-11
**Author**: Claude Code
**Status**: In Development

---

## 1. Executive Summary

WSOPTV는 18TB+ 포커 방송 아카이브를 위한 **초대 기반 프라이빗 VOD 스트리밍 플랫폼**입니다. Windows Native 환경에서 NAS의 미디어 파일을 직접 스트리밍하며, 4-Tier 캐싱 시스템으로 다중 동시 접속을 안정적으로 지원합니다.

### 1.1 Vision

> "포커 팬들이 좋아하는 플레이어의 명장면을 빠르게 찾고, 끊김 없이 시청하는 프라이빗 커뮤니티 플랫폼"

### 1.2 Core Values

| 가치 | 설명 |
|------|------|
| **Simple** | 복잡한 미들웨어 없이 직접 스트리밍 |
| **Fast** | 4-Tier 캐싱으로 즉시 재생 |
| **Private** | 초대 기반, 관리자 승인 시스템 |
| **Reliable** | Rate Limiting으로 안정적 동시 접속 |

---

## 2. Problem Statement

### 2.1 현재 Pain Points

| 문제 | 설명 |
|------|------|
| **접근성** | 18TB+ 아카이브가 NAS에 산재, 체계적 탐색 불가 |
| **검색** | 특정 플레이어나 핸드를 찾으려면 전체 영상 시청 필요 |
| **동시성** | 여러 명이 동시에 NAS 접근 시 병목 발생 |
| **보안** | 공개 플랫폼 부적합, 제한된 커뮤니티 필요 |

### 2.2 Target Users

**Primary**: 초대받은 포커 팬 (10-30명)
- WSOP, HCL, PAD 등 포커 방송 시청
- 특정 플레이어 팔로우
- 명장면 반복 시청

**Secondary**: 관리자 (1-2명)
- 사용자 승인/관리
- 시스템 모니터링
- 콘텐츠 관리

---

## 3. Solution Architecture

### 3.1 System Overview

```
┌─────────────────────────────────────────────────────────────────────────┐
│                         WSOPTV Platform                                  │
│                    Windows Native Architecture                           │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│    ┌─────────────┐         ┌─────────────┐         ┌─────────────┐     │
│    │   Browser   │────────▶│   Web App   │────────▶│   API       │     │
│    │   (User)    │         │   :3000     │         │   :8000     │     │
│    └─────────────┘         └─────────────┘         └──────┬──────┘     │
│                                                           │             │
│                                                           ▼             │
│    ┌────────────────────────────────────────────────────────────────┐  │
│    │                    4-Tier Cache System                          │  │
│    │  ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐        │  │
│    │  │ Redis   │──▶│ Local   │──▶│  Rate   │──▶│   NAS   │        │  │
│    │  │ (Meta)  │   │  SSD    │   │ Limiter │   │  (SMB)  │        │  │
│    │  │  L1     │   │  L2     │   │   L3    │   │   L4    │        │  │
│    │  └─────────┘   └─────────┘   └─────────┘   └─────────┘        │  │
│    └────────────────────────────────────────────────────────────────┘  │
│                                                                          │
│    ┌─────────────┐         ┌─────────────┐         ┌─────────────┐     │
│    │ PostgreSQL  │         │ MeiliSearch │         │   FFmpeg    │     │
│    │  :5432      │         │   :7700     │         │  (Worker)   │     │
│    └─────────────┘         └─────────────┘         └─────────────┘     │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    │ SMB 3.0
                                    ▼
                            ┌─────────────┐
                            │     NAS     │
                            │   18TB+     │
                            └─────────────┘
```

### 3.2 Technology Stack

| Layer | Technology | Purpose |
|-------|------------|---------|
| **Frontend** | SvelteKit 2, TypeScript | UI, Video Player |
| **Backend** | FastAPI, Python 3.12 | API, Streaming |
| **Database** | PostgreSQL 16 | Metadata, Users |
| **Search** | MeiliSearch 1.11 | Full-text Search |
| **Cache** | Redis 7 | L1 Cache, Sessions |
| **Storage** | Local SSD (500GB+) | L2 Hot Cache |
| **Media** | NAS (SMB 3.0) | Original Files |
| **Thumbnail** | FFmpeg 7 | Image Generation |

### 3.3 Directory Structure

```
D:\WSOPTV\
├── web\                    # SvelteKit Frontend
│   ├── src\
│   │   ├── routes\
│   │   │   ├── +page.svelte           # Home
│   │   │   ├── watch\[id]\            # Player
│   │   │   ├── search\                # Search
│   │   │   ├── login\                 # Login
│   │   │   └── admin\                 # Admin Dashboard
│   │   └── lib\
│   │       ├── components\
│   │       ├── stores\
│   │       └── api\
│   └── package.json
│
├── api\                    # FastAPI Backend
│   ├── app\
│   │   ├── main.py
│   │   ├── routers\
│   │   │   ├── auth.py
│   │   │   ├── contents.py
│   │   │   ├── stream.py
│   │   │   └── admin.py
│   │   ├── services\
│   │   │   ├── cache_service.py
│   │   │   ├── stream_service.py
│   │   │   └── thumbnail_service.py
│   │   ├── models\
│   │   └── core\
│   │       ├── config.py
│   │       └── security.py
│   └── requirements.txt
│
├── cache\                  # Local SSD Cache
│   ├── hot\                # Video files
│   └── thumbnails\         # Generated images
│
├── data\                   # PostgreSQL data
│
└── scripts\                # Management scripts
    ├── start.ps1
    ├── stop.ps1
    └── backup.ps1
```

---

## 4. 4-Tier Cache System

### 4.1 Overview

```
┌──────────────────────────────────────────────────────────────────────────┐
│                        4-Tier Cache Hierarchy                             │
├──────────────────────────────────────────────────────────────────────────┤
│                                                                           │
│  Request ──▶ [L1: Redis] ──miss──▶ [L2: SSD] ──miss──▶ [L3: Rate Limit] │
│                   │                     │                      │          │
│                  hit                   hit                   pass         │
│                   │                     │                      │          │
│                   ▼                     ▼                      ▼          │
│              Response             Response              [L4: NAS]        │
│                                                              │            │
│                                                          Response        │
│                                                                           │
└──────────────────────────────────────────────────────────────────────────┘
```

### 4.2 L1: Redis Cache (Hot Metadata)

**Purpose**: 메타데이터, 세션, 실시간 상태 캐싱

| Key Pattern | TTL | Data |
|-------------|-----|------|
| `content:{id}` | 10분 | 콘텐츠 메타데이터 |
| `home:rows` | 5분 | 홈페이지 Row 데이터 |
| `user:{id}:session` | 24시간 | 사용자 세션 |
| `user:{id}:progress:{cid}` | 7일 | 시청 진행률 |
| `stream:active` | 실시간 | 활성 스트림 카운트 |
| `stream:user:{id}` | 실시간 | 사용자별 스트림 카운트 |

```python
# Redis Cache Service
class RedisCache:
    async def get_content(self, content_id: str) -> Content | None:
        cached = await self.redis.get(f"content:{content_id}")
        if cached:
            return Content.model_validate_json(cached)
        return None

    async def set_content(self, content: Content, ttl: int = 600):
        await self.redis.set(
            f"content:{content.id}",
            content.model_dump_json(),
            ex=ttl
        )
```

### 4.3 L2: Local SSD Cache (Hot Content)

**Purpose**: 자주 접근하는 비디오 파일을 SSD에 복사하여 NAS 부하 분산

| 설정 | 값 | 설명 |
|------|-----|------|
| 경로 | `D:\WSOPTV\cache\hot\` | SSD 캐시 디렉토리 |
| 최대 크기 | 500GB | LRU eviction 기준 |
| 캐시 조건 | 7일 내 5회+ 조회 | Hot content 기준 |
| 정책 | LRU | Least Recently Used |

```python
# SSD Cache Manager
class SSDCacheManager:
    def __init__(self, cache_dir: Path, max_size_gb: int = 500):
        self.cache_dir = cache_dir
        self.max_bytes = max_size_gb * 1024 ** 3

    async def get_path(self, content_id: str, nas_path: str) -> Path:
        """캐시된 경로 반환 또는 NAS 경로 반환"""
        cached = self.cache_dir / f"{content_id}.mp4"

        if cached.exists():
            # 접근 시간 업데이트 (LRU)
            cached.touch()
            return cached

        # Hot content면 백그라운드 캐싱 예약
        if await self._is_hot(content_id):
            asyncio.create_task(self._cache_file(content_id, nas_path))

        return Path(nas_path)

    async def _is_hot(self, content_id: str) -> bool:
        """최근 7일 내 5회 이상 조회 여부"""
        count = await db.scalar(
            select(func.count()).where(
                ViewEvent.content_id == content_id,
                ViewEvent.created_at > datetime.now() - timedelta(days=7)
            )
        )
        return count >= 5

    async def _cache_file(self, content_id: str, nas_path: str):
        """NAS → SSD 복사 (저속, 백그라운드)"""
        await self._ensure_space(Path(nas_path).stat().st_size)
        # robocopy로 저속 복사 (NAS 부하 최소화)
        await asyncio.subprocess.create_subprocess_exec(
            "robocopy", Path(nas_path).parent, self.cache_dir,
            Path(nas_path).name, "/Z", "/IPG:100"  # 재시작 가능, 100ms 갭
        )

    async def _ensure_space(self, required: int):
        """LRU 방식으로 공간 확보"""
        while self._current_size() + required > self.max_bytes:
            oldest = min(self.cache_dir.glob("*.mp4"), key=lambda p: p.stat().st_atime)
            oldest.unlink()
```

### 4.4 L3: Rate Limiter (Concurrency Control)

**Purpose**: NAS 동시 접속 제한으로 대역폭 보호

| 제한 | 값 | 설명 |
|------|-----|------|
| 전체 동시 스트림 | 20 | 서버 전체 |
| 사용자당 스트림 | 3 | 개별 사용자 |
| 대기열 | 10 | 초과 시 대기 |

```python
# Rate Limiter
class StreamRateLimiter:
    MAX_GLOBAL = 20
    MAX_PER_USER = 3

    async def acquire(self, user_id: str) -> tuple[bool, str]:
        """스트림 슬롯 획득 시도"""
        global_count = int(await self.redis.get("stream:active") or 0)
        user_count = int(await self.redis.get(f"stream:user:{user_id}") or 0)

        if global_count >= self.MAX_GLOBAL:
            return False, "서버 동시 접속 한도 초과. 잠시 후 다시 시도하세요."

        if user_count >= self.MAX_PER_USER:
            return False, f"동시 재생 {self.MAX_PER_USER}개 초과"

        # 슬롯 획득
        pipe = self.redis.pipeline()
        pipe.incr("stream:active")
        pipe.incr(f"stream:user:{user_id}")
        pipe.expire("stream:active", 3600)
        pipe.expire(f"stream:user:{user_id}", 3600)
        await pipe.execute()

        return True, "OK"

    async def release(self, user_id: str):
        """스트림 슬롯 반환"""
        pipe = self.redis.pipeline()
        pipe.decr("stream:active")
        pipe.decr(f"stream:user:{user_id}")
        await pipe.execute()
```

### 4.5 L4: NAS (Cold Storage)

**Purpose**: 원본 비디오 파일 저장소

| 설정 | 값 |
|------|-----|
| Protocol | SMB 3.0 |
| Host | `\\10.10.100.122\GGPNAs` |
| Mount | `Z:\ARCHIVE` |
| Bandwidth | 1Gbps (≈125MB/s) |

```powershell
# NAS 마운트 스크립트 (scripts/mount-nas.ps1)
$nasPath = "\\10.10.100.122\GGPNAs"
$drive = "Z:"

if (-not (Test-Path $drive)) {
    net use $drive $nasPath /persistent:yes
    Write-Host "NAS mounted at $drive" -ForegroundColor Green
}
```

---

## 5. Core Features

### 5.1 Authentication (관리자 승인 방식)

#### 5.1.1 초기 관리자 계정

플랫폼 설치 시 기본 관리자 계정이 생성됩니다:

| 필드 | 값 |
|------|-----|
| **Username** | `garimto` |
| **Password** | `1234` |
| **Role** | `admin` |
| **Status** | `active` |

> ⚠️ 보안: 프로덕션 환경에서 반드시 비밀번호를 변경하세요.

#### 5.1.2 회원가입 플로우

```
┌─────────────────────────────────────────────────────────────────────────┐
│                         회원가입 Flow                                     │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   [회원가입 페이지]                                                      │
│   ┌────────────────────┐                                                │
│   │  아이디 (username) │  ← 영문/숫자, 4-20자                          │
│   ├────────────────────┤                                                │
│   │  비밀번호          │  ← 8자 이상                                    │
│   ├────────────────────┤                                                │
│   │  비밀번호 확인     │  ← 일치 여부 검증                              │
│   ├────────────────────┤                                                │
│   │     [가입하기]     │                                                │
│   └────────────────────┘                                                │
│              │                                                          │
│              ▼                                                          │
│   ┌────────────────────┐                                                │
│   │  승인 대기 페이지  │  "관리자 승인을 기다려주세요"                  │
│   └────────────────────┘                                                │
│              │                                                          │
│              ▼                                                          │
│   ┌────────────────────┐         ┌────────────────────┐                │
│   │ 관리자 대시보드    │ ──────▶ │    승인/거부       │                │
│   │ (Pending Users)    │         └────────────────────┘                │
│   └────────────────────┘                                                │
│              │                                                          │
│       ┌──────┴──────┐                                                   │
│       ▼             ▼                                                   │
│  [승인됨]      [거부됨]                                                 │
│  status:      status:                                                   │
│  active       rejected                                                  │
│       │             │                                                   │
│       ▼             ▼                                                   │
│  [로그인 가능]  [로그인 불가]                                           │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

#### 5.1.3 사용자 상태 (User Status)

| 상태 | 설명 | 로그인 | 페이지 접근 |
|------|------|--------|-------------|
| `pending` | 승인 대기 | ❌ | 승인 대기 페이지만 |
| `active` | 활성 | ✅ | 전체 접근 가능 |
| `rejected` | 거부됨 | ❌ | 거부 안내 페이지 |
| `suspended` | 정지됨 | ❌ | 정지 안내 페이지 |

**User Model**:
```python
class UserStatus(str, Enum):
    PENDING = "pending"      # 승인 대기
    ACTIVE = "active"        # 활성
    REJECTED = "rejected"    # 거부
    SUSPENDED = "suspended"  # 정지

class User(Base):
    __tablename__ = "users"

    id: Mapped[str] = mapped_column(primary_key=True)
    username: Mapped[str] = mapped_column(unique=True)
    email: Mapped[str] = mapped_column(unique=True)
    password_hash: Mapped[str]
    display_name: Mapped[str | None]
    status: Mapped[UserStatus] = mapped_column(default=UserStatus.PENDING)
    is_admin: Mapped[bool] = mapped_column(default=False)
    created_at: Mapped[datetime] = mapped_column(default=datetime.utcnow)
    approved_at: Mapped[datetime | None]
    approved_by: Mapped[str | None]
```

**API Endpoints**:
```
POST /api/auth/register     # 회원가입 (Pending 상태)
POST /api/auth/login        # 로그인 (Active만 가능)
POST /api/auth/logout       # 로그아웃
GET  /api/auth/me           # 현재 사용자 정보
```

### 5.2 Content Browsing (Netflix-style)

#### 5.2.1 전체 레이아웃 구조

```
┌──────────────────────────────────────────────────────────────────────────────┐
│  ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓ HEADER (투명 그라디언트) ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓  │
│                                                                              │
│  WSOPTV    All  WSOP  HCL  GGMillions  GOG  MPP  PAD        🔍   Admin  [G▼]│
│                                                                              │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░│ │
│  │░░                                                                   ░░│ │
│  │░░   WSOPTV ORIGINAL                                                 ░░│ │
│  │░░                                                                   ░░│ │
│  │░░   ██╗    ██╗███████╗ ██████╗ ██████╗                              ░░│ │
│  │░░   ██║    ██║██╔════╝██╔═══██╗██╔══██╗  2024 Main Event            ░░│ │
│  │░░   ██║ █╗ ██║███████╗██║   ██║██████╔╝  Final Table                ░░│ │
│  │░░   ██║███╗██║╚════██║██║   ██║██╔═══╝                              ░░│ │
│  │░░   ╚███╔███╔╝███████║╚██████╔╝██║                                  ░░│ │
│  │░░                                                                   ░░│ │
│  │░░   Phil Ivey vs Daniel Negreanu - $10M 팟의 역대급 블러프           ░░│ │
│  │░░                                                                   ░░│ │
│  │░░   ┌──────────────┐  ┌──────────────┐                              ░░│ │
│  │░░   │  ▶  재생     │  │  ℹ  상세정보  │                              ░░│ │
│  │░░   └──────────────┘  └──────────────┘                              ░░│ │
│  │░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░│ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                    ← Hero Billboard (화면 40%, 좌측 그라디언트)              │
│                                                                              │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  이어서 시청하기                                                        ▶   │
│  ──────────────────────────────────────────────────────────────────────────  │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐   │
│  │ ░░░░░░░ │ │ ░░░░░░░ │ │ ░░░░░░░ │ │ ░░░░░░░ │ │ ░░░░░░░ │ │ ░░░░░░░ │   │
│  │ ░░▶░░░ │ │ ░░▶░░░ │ │ ░░▶░░░ │ │ ░░▶░░░ │ │ ░░▶░░░ │ │ ░░▶░░░ │   │
│  │ ▓▓▓▓▓▓▓ │ │ ▓▓▓░░░░ │ │ ▓▓░░░░░ │ │ ▓▓▓▓░░░ │ │ ▓░░░░░░ │ │ ▓▓▓▓▓░░ │   │
│  │ 45%     │ │ 30%     │ │ 15%     │ │ 60%     │ │ 5%      │ │ 80%     │   │
│  └─────────┘ └─────────┘ └─────────┘ └─────────┘ └─────────┘ └─────────┘   │
│   HCL S14    WSOP 2023   GOG Ep.45   PAD Cash    GGM Final   MPP Tour       │
│                                                                        ◀──▶ │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  🔥 오늘 WSOPTV TOP 10                                                  ▶   │
│  ──────────────────────────────────────────────────────────────────────────  │
│ ██╗  ┌───────┐ ██████╗ ┌───────┐ ███████╗┌───────┐ ██╗  ██╗┌───────┐       │
│ ███║ │ ░░░░░ │ ╚════██╗│ ░░░░░ │ ╚════██║│ ░░░░░ │ ██║  ██║│ ░░░░░ │       │
│ ╚██║ │ ░▶░░ │  █████╔╝│ ░▶░░ │     ██╔╝│ ░▶░░ │ ███████║│ ░▶░░ │ ...   │
│  ██║ │ ░░░░░ │  ╚═══██╗│ ░░░░░ │    ██╔╝ │ ░░░░░ │ ╚════██║│ ░░░░░ │       │
│  ██║ └───────┘ ██████╔╝└───────┘    ██║  └───────┘      ██║└───────┘       │
│  ╚═╝ Phil Ivey ╚═════╝  Negreanu    ╚═╝  Tom Dwan       ╚═╝ GG Squad       │
│      블러프          올인콜             쿨러핸드           토너먼트      ◀──▶ │
│                                                                              │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  WSOP 시리즈                                                            ▶   │
│  ──────────────────────────────────────────────────────────────────────────  │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐   │
│  │ ░░░░░░░ │ │ ░░░░░░░ │ │ ░░░░░░░ │ │ ░░░░░░░ │ │ ░░░░░░░ │ │ ░░░░░░░ │   │
│  │ ░░▶░░░ │ │ ░░▶░░░ │ │ ░░▶░░░ │ │ ░░▶░░░ │ │ ░░▶░░░ │ │ ░░▶░░░ │   │
│  └─────────┘ └─────────┘ └─────────┘ └─────────┘ └─────────┘ └─────────┘   │
│   2024 Main   2024 $25K   2023 Main   2023 $50K   2022 Main   2022 HU       │
│                                                                        ◀──▶ │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  HCL (Hustler Casino Live)                                              ▶   │
│  ──────────────────────────────────────────────────────────────────────────  │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐   │
│  │ ░░░░░░░ │ │ ░░░░░░░ │ │ ░░░░░░░ │ │ ░░░░░░░ │ │ ░░░░░░░ │ │ ░░░░░░░ │   │
│  │ ░░▶░░░ │ │ ░░▶░░░ │ │ ░░▶░░░ │ │ ░░▶░░░ │ │ ░░▶░░░ │ │ ░░▶░░░ │   │
│  └─────────┘ └─────────┘ └─────────┘ └─────────┘ └─────────┘ └─────────┘   │
│   S14 Ep.10  S14 Ep.09   S14 Ep.08   S14 Ep.07   S14 Ep.06   S14 Ep.05     │
│                                                                        ◀──▶ │
├──────────────────────────────────────────────────────────────────────────────┤
│  GGMillions, PAD, GOG, MPP 등 카테고리별 Row 추가...                         │
└──────────────────────────────────────────────────────────────────────────────┘
```

#### 5.2.2 Hero Billboard 상세

화면 상단 40%를 차지하는 대형 배너로 추천 콘텐츠를 강조합니다.

| 요소 | 설명 |
|------|------|
| **배경** | 콘텐츠 썸네일 풀스크린 (블러 처리) |
| **그라디언트** | 좌측에서 우측으로 어두워지는 오버레이 |
| **콘텐츠 정보** | 로고/제목, 설명 (2줄), 메타 정보 |
| **액션 버튼** | [▶ 재생] [ℹ 상세정보] |
| **자동 전환** | 10초마다 다음 추천 콘텐츠로 전환 |

#### 5.2.3 Row 레이아웃

각 Row는 가로 스크롤 가능한 콘텐츠 카드 목록입니다.

| Row 타입 | 설명 | 정렬 기준 |
|----------|------|----------|
| **이어서 시청하기** | 시청 중단 지점이 있는 콘텐츠 | 최근 시청 순 |
| **TOP 10** | 조회수 기준 인기 콘텐츠 (큰 순위 숫자) | 조회수 내림차순 |
| **카테고리별** | WSOP, HCL, GGMillions 등 | 최신 등록 순 |
| **최근 추가** | 신규 등록 콘텐츠 | 등록일 내림차순 |

#### 5.2.4 카드 Hover 확대 효과

```
[기본 상태]                         [Hover 상태 - 150% 확대]

┌─────────┐                        ┌─────────────────────────┐
│ ░░░░░░░ │                        │ ░░░░░░░░░░░░░░░░░░░░░░░ │
│ ░░▶░░░ │          ───▶          │ ░░░░░░░░░░░░░░░░░░░░░░░ │
│ ░░░░░░░ │                        │ ░░░░░░  ▶  ░░░░░░░░░░░ │
└─────────┘                        │ ░░░░░░░░░░░░░░░░░░░░░░░ │
 WSOP 2024                         │ ░░░░░░░░░░░░░░░░░░░░░░░ │
                                   ├─────────────────────────┤
                                   │ WSOP 2024 Main Event    │
                                   │ Final Table             │
                                   │                         │
                                   │ ▶  ⊕  👍  👎  ▼        │
                                   │                         │
                                   │ 3h 45m │ HD │ 2024      │
                                   │ 포커 • 토너먼트 • 하이스테이크 │
                                   └─────────────────────────┘
```

| 상태 | 동작 |
|------|------|
| **기본** | 썸네일 + 제목 |
| **Hover (0.3초)** | 150% 확대, 그림자 강화 |
| **Hover (1초 후)** | 미리보기 영상 자동재생 (음소거) |
| **Hover 상세** | 제목, 메타정보, 액션 버튼, 장르 태그 |

#### 5.2.5 색상 팔레트

| 용도 | 색상 코드 | 설명 |
|------|----------|------|
| **배경** | `#141414` | 메인 배경 |
| **카드 배경** | `#181818` | 카드/Row 배경 |
| **카드 호버** | `#2F2F2F` | 호버 시 카드 배경 |
| **강조 (Red)** | `#E50914` | 로고, 버튼, 액센트 |
| **성공 (Green)** | `#46D369` | 진행률 바, 카테고리 뱃지 |
| **텍스트 (White)** | `#FFFFFF` | 메인 텍스트 |
| **텍스트 (Gray)** | `#B3B3B3` | 서브 텍스트 |
| **텍스트 (Muted)** | `#808080` | 비활성 텍스트 |

#### 5.2.6 구현 컴포넌트

```typescript
// 핵심 컴포넌트 구조
components/
├── HeroBillboard/
│   ├── HeroBillboard.tsx      // 메인 배너
│   ├── HeroContent.tsx        // 콘텐츠 정보
│   └── HeroControls.tsx       // 재생/상세 버튼
├── ContentRow/
│   ├── ContentRow.tsx         // Row 컨테이너
│   ├── RowHeader.tsx          // Row 제목
│   └── ScrollControls.tsx     // 좌우 스크롤 버튼
├── ContentCard/
│   ├── ContentCard.tsx        // 기본 카드
│   ├── CardHover.tsx          // Hover 확대 상태
│   ├── CardPreview.tsx        // 미리보기 영상
│   └── TopTenCard.tsx         // TOP 10 순위 카드
└── ProgressBar/
    └── ProgressBar.tsx        // 시청 진행률 바
```

**Row Types**:
```python
class RowType(str, Enum):
    CONTINUE_WATCHING = "continue_watching"  # 이어보기
    RECENTLY_ADDED = "recently_added"        # 최근 추가
    CATALOG = "catalog"                      # 카탈로그별 (WSOP, HCL, PAD...)
    TRENDING = "trending"                    # 인기 콘텐츠

class RowData(BaseModel):
    id: str
    type: RowType
    title: str
    items: list[ContentCard]
    view_all_url: str | None = None
```

**API Endpoints**:
```
GET /api/home                    # 홈페이지 Row 목록
GET /api/contents                # 콘텐츠 목록 (필터/페이지네이션)
GET /api/contents/{id}           # 콘텐츠 상세
GET /api/catalogs                # 카탈로그 목록
GET /api/search?q={query}        # 검색
```

### 5.3 Video Streaming (Direct Play)

**HTTP Range Request 기반 Direct Play**:
- 트랜스코딩 없이 원본 파일 직접 스트리밍
- 브라우저 네이티브 `<video>` 태그 사용
- Seek 시 즉시 해당 위치로 이동

```python
@router.get("/api/stream/{content_id}")
async def stream(
    content_id: str,
    request: Request,
    user: User = Depends(get_current_user),
    rate_limiter: StreamRateLimiter = Depends(),
    cache: SSDCacheManager = Depends()
):
    # 1. Rate Limit 확인
    allowed, msg = await rate_limiter.acquire(user.id)
    if not allowed:
        raise HTTPException(429, msg)

    # 2. 파일 경로 결정 (SSD 캐시 또는 NAS)
    content = await get_content(content_id)
    file_path = await cache.get_path(content_id, content.nas_path)

    # 3. Range 헤더 처리
    range_header = request.headers.get("range")
    file_size = file_path.stat().st_size

    if range_header:
        # Partial Content (206)
        start, end = parse_range(range_header, file_size)
        return StreamingResponse(
            stream_file(file_path, start, end),
            status_code=206,
            headers={
                "Content-Range": f"bytes {start}-{end}/{file_size}",
                "Accept-Ranges": "bytes",
                "Content-Type": "video/mp4"
            }
        )
    else:
        # Full Content (200)
        return StreamingResponse(
            stream_file(file_path, 0, file_size - 1),
            headers={
                "Accept-Ranges": "bytes",
                "Content-Type": "video/mp4"
            }
        )
```

**Frontend Video Player**:
```svelte
<!-- VideoPlayer.svelte -->
<script lang="ts">
  export let contentId: string;
  export let startTime = 0;

  let video: HTMLVideoElement;

  onMount(() => {
    video.currentTime = startTime;

    // 30초마다 진행률 저장
    const interval = setInterval(saveProgress, 30000);
    return () => clearInterval(interval);
  });

  async function saveProgress() {
    await fetch(`/api/progress/${contentId}`, {
      method: 'PUT',
      body: JSON.stringify({
        position: video.currentTime,
        duration: video.duration
      })
    });
  }
</script>

<video
  bind:this={video}
  src="/api/stream/{contentId}"
  controls
  autoplay
/>
```

### 5.4 Watch Progress (이어보기)

```python
class WatchProgress(Base):
    __tablename__ = "watch_progress"

    user_id: Mapped[str] = mapped_column(primary_key=True)
    content_id: Mapped[str] = mapped_column(primary_key=True)
    position: Mapped[float]          # 현재 위치 (초)
    duration: Mapped[float]          # 전체 길이 (초)
    progress_percent: Mapped[int]    # 진행률 (0-100)
    updated_at: Mapped[datetime]

# API
@router.put("/api/progress/{content_id}")
async def update_progress(content_id: str, data: ProgressUpdate, user: User = Depends()):
    progress = await get_or_create_progress(user.id, content_id)
    progress.position = data.position
    progress.duration = data.duration
    progress.progress_percent = int((data.position / data.duration) * 100)
    progress.updated_at = datetime.utcnow()
    await db.commit()

@router.get("/api/progress/{content_id}")
async def get_progress(content_id: str, user: User = Depends()):
    progress = await get_progress(user.id, content_id)
    return {"position": progress.position if progress else 0}
```

### 5.5 Thumbnail Generation (On-Demand)

```python
class ThumbnailService:
    def __init__(self, cache_dir: Path):
        self.cache_dir = cache_dir

    async def get_poster(self, content_id: str, video_path: str) -> Path:
        """10초 지점 프레임 추출"""
        output = self.cache_dir / content_id / "poster.jpg"

        if output.exists():
            return output

        output.parent.mkdir(parents=True, exist_ok=True)

        # FFmpeg으로 프레임 추출
        proc = await asyncio.create_subprocess_exec(
            "ffmpeg", "-y",
            "-ss", "10",
            "-i", video_path,
            "-vframes", "1",
            "-vf", "scale=480:-1",
            "-q:v", "2",
            str(output)
        )
        await proc.wait()

        return output
```

---

## 6. Admin Dashboard

### 6.1 Features

```
┌─────────────────────────────────────────────────────────────────┐
│  WSOPTV Admin                                    [user@admin]  │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  System Status                                                  │
│  ┌────────────────────────────────────────────────────────────┐│
│  │  Active Streams   12/20  ████████████░░░░░░  60%           ││
│  │  SSD Cache       423/500GB  ████████████████░░  85%        ││
│  │  NAS Status      ● Online  (32ms latency)                  ││
│  └────────────────────────────────────────────────────────────┘│
│                                                                  │
│  Pending Users (3)                                              │
│  ┌────────────────────────────────────────────────────────────┐│
│  │  Username    Email              Registered    Actions      ││
│  │  ─────────────────────────────────────────────────────────││
│  │  newuser1    new@email.com      2 hours ago   [✓] [✗]     ││
│  │  newuser2    test@test.com      1 day ago     [✓] [✗]     ││
│  │  newuser3    foo@bar.com        3 days ago    [✓] [✗]     ││
│  └────────────────────────────────────────────────────────────┘│
│                                                                  │
│  Active Streams                                                 │
│  ┌────────────────────────────────────────────────────────────┐│
│  │  User        Content              Duration    Bandwidth    ││
│  │  ─────────────────────────────────────────────────────────││
│  │  user1       WSOP 2024 ME Final   1h 23m      15.2 Mbps   ││
│  │  user2       HCL Episode 145      45m         12.8 Mbps   ││
│  │  user3       PAD S2E3             2h 10m      18.4 Mbps   ││
│  └────────────────────────────────────────────────────────────┘│
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### 6.2 Admin API

```
GET  /api/admin/status           # 시스템 상태
GET  /api/admin/users            # 사용자 목록
GET  /api/admin/users/pending    # 승인 대기 사용자
POST /api/admin/users/{id}/approve  # 승인
POST /api/admin/users/{id}/reject   # 거부
GET  /api/admin/streams          # 활성 스트림 목록
GET  /api/admin/cache            # 캐시 상태
```

---

## 7. Data Models

### 7.1 Database Schema

```sql
-- Users
CREATE TABLE users (
    id VARCHAR(36) PRIMARY KEY,
    username VARCHAR(50) UNIQUE NOT NULL,
    email VARCHAR(100) UNIQUE NOT NULL,
    password_hash VARCHAR(255) NOT NULL,
    display_name VARCHAR(100),
    status VARCHAR(20) DEFAULT 'pending',
    is_admin BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP DEFAULT NOW(),
    approved_at TIMESTAMP,
    approved_by VARCHAR(36) REFERENCES users(id)
);

-- Catalogs (WSOP, HCL, PAD, etc.)
CREATE TABLE catalogs (
    id VARCHAR(36) PRIMARY KEY,
    name VARCHAR(100) NOT NULL,
    description TEXT,
    thumbnail_path VARCHAR(500),
    sort_order INT DEFAULT 0,
    created_at TIMESTAMP DEFAULT NOW()
);

-- Contents (Videos)
CREATE TABLE contents (
    id VARCHAR(36) PRIMARY KEY,
    catalog_id VARCHAR(36) REFERENCES catalogs(id),
    title VARCHAR(200) NOT NULL,
    description TEXT,
    nas_path VARCHAR(500) NOT NULL,  -- NAS 파일 경로
    duration INT,                     -- 초 단위
    file_size BIGINT,
    thumbnail_path VARCHAR(500),
    created_at TIMESTAMP DEFAULT NOW(),
    added_at TIMESTAMP DEFAULT NOW()
);

-- Watch Progress
CREATE TABLE watch_progress (
    user_id VARCHAR(36) REFERENCES users(id),
    content_id VARCHAR(36) REFERENCES contents(id),
    position FLOAT NOT NULL,
    duration FLOAT NOT NULL,
    progress_percent INT,
    updated_at TIMESTAMP DEFAULT NOW(),
    PRIMARY KEY (user_id, content_id)
);

-- View Events (for trending & hot content detection)
CREATE TABLE view_events (
    id SERIAL PRIMARY KEY,
    user_id VARCHAR(36) REFERENCES users(id),
    content_id VARCHAR(36) REFERENCES contents(id),
    created_at TIMESTAMP DEFAULT NOW()
);

-- Indexes
CREATE INDEX idx_contents_catalog ON contents(catalog_id);
CREATE INDEX idx_progress_user ON watch_progress(user_id);
CREATE INDEX idx_view_events_content_date ON view_events(content_id, created_at);
```

### 7.2 MeiliSearch Index

```python
# 검색 인덱스 설정
contents_index = {
    "primaryKey": "id",
    "searchableAttributes": [
        "title",
        "description",
        "catalog_name"
    ],
    "filterableAttributes": [
        "catalog_id",
        "duration"
    ],
    "sortableAttributes": [
        "added_at",
        "title"
    ]
}
```

---

## 8. API Specification

### 8.1 Authentication

```yaml
POST /api/auth/register:
  request:
    username: string (required)
    email: string (required)
    password: string (required)
    display_name: string (optional)
  response:
    message: "회원가입 완료. 관리자 승인을 기다려주세요."

POST /api/auth/login:
  request:
    username: string
    password: string
  response:
    access_token: string
    user:
      id: string
      username: string
      display_name: string
      is_admin: boolean
  errors:
    401: "잘못된 인증 정보"
    403: "승인 대기 중" | "계정 거부됨" | "계정 정지됨"
```

### 8.2 Contents

```yaml
GET /api/home:
  response:
    rows:
      - id: string
        type: "continue_watching" | "recently_added" | "catalog" | "trending"
        title: string
        items: ContentCard[]
        view_all_url: string | null

GET /api/contents:
  params:
    catalog_id: string (optional)
    page: int (default: 1)
    limit: int (default: 20)
  response:
    items: Content[]
    total: int
    page: int
    pages: int

GET /api/contents/{id}:
  response:
    id: string
    title: string
    description: string
    duration: int
    thumbnail_url: string
    catalog:
      id: string
      name: string
    progress: float | null  # 로그인 시
```

### 8.3 Streaming

```yaml
GET /api/stream/{content_id}:
  headers:
    Range: "bytes=0-1023" (optional)
  response:
    200: Full video stream
    206: Partial content
    429: Rate limit exceeded

PUT /api/progress/{content_id}:
  request:
    position: float
    duration: float
  response:
    success: true
```

---

## 9. Configuration

### 9.1 Environment Variables

```env
# Database
DATABASE_URL=postgresql://wsoptv:password@localhost:5432/wsoptv

# Redis
REDIS_URL=redis://localhost:6379

# MeiliSearch
MEILI_HOST=http://localhost:7700
MEILI_KEY=your_master_key

# NAS
NAS_PATH=Z:\ARCHIVE

# Cache
SSD_CACHE_PATH=D:\WSOPTV\cache\hot
SSD_CACHE_MAX_GB=500
THUMBNAIL_CACHE_PATH=D:\WSOPTV\cache\thumbnails

# Rate Limiting
MAX_GLOBAL_STREAMS=20
MAX_USER_STREAMS=3

# Security
JWT_SECRET=your_jwt_secret
JWT_EXPIRE_HOURS=24

# Server
API_HOST=0.0.0.0
API_PORT=8000
WEB_PORT=3000
```

### 9.2 Service Management

```powershell
# scripts/start.ps1
Write-Host "Starting WSOPTV..." -ForegroundColor Cyan

# Mount NAS
& .\mount-nas.ps1

# Start PostgreSQL (Windows Service)
Start-Service postgresql-x64-16

# Start Redis
Start-Process redis-server -WindowStyle Hidden

# Start MeiliSearch
Start-Process meilisearch -ArgumentList "--env=production" -WindowStyle Hidden

# Start API (FastAPI)
Set-Location D:\WSOPTV\api
Start-Process python -ArgumentList "-m", "uvicorn", "app.main:app", "--host", "0.0.0.0", "--port", "8000" -WindowStyle Hidden

# Start Web (SvelteKit)
Set-Location D:\WSOPTV\web
Start-Process npm -ArgumentList "run", "preview" -WindowStyle Hidden

Write-Host "WSOPTV Started!" -ForegroundColor Green
Write-Host "Web: http://localhost:3000" -ForegroundColor Yellow
Write-Host "API: http://localhost:8000" -ForegroundColor Yellow
```

---

## 10. Implementation Roadmap

### Phase 1: Foundation (Week 1-2)

| Task | Priority | Days |
|------|----------|------|
| 프로젝트 구조 생성 | P0 | 1 |
| PostgreSQL 설정 + 스키마 | P0 | 1 |
| Redis + MeiliSearch 설정 | P0 | 1 |
| FastAPI 기본 구조 | P0 | 2 |
| SvelteKit 기본 구조 | P0 | 2 |
| NAS 마운트 자동화 | P0 | 1 |

### Phase 2: Authentication (Week 3)

| Task | Priority | Days |
|------|----------|------|
| User 모델 + 마이그레이션 | P0 | 1 |
| Register/Login API | P0 | 2 |
| JWT 토큰 관리 | P0 | 1 |
| 관리자 승인 API | P0 | 1 |
| 로그인/가입 UI | P0 | 2 |

### Phase 3: Core Features (Week 4-5)

| Task | Priority | Days |
|------|----------|------|
| Contents 모델 + API | P0 | 2 |
| 홈페이지 Row API | P0 | 2 |
| 검색 API (MeiliSearch) | P0 | 2 |
| 홈페이지 UI | P0 | 2 |
| 콘텐츠 브라우징 UI | P0 | 2 |

### Phase 4: Streaming (Week 6-7)

| Task | Priority | Days |
|------|----------|------|
| L1 Redis 캐시 | P0 | 1 |
| L2 SSD 캐시 매니저 | P0 | 3 |
| L3 Rate Limiter | P0 | 2 |
| HTTP Range Streaming | P0 | 2 |
| Video Player UI | P0 | 2 |
| 시청 진행률 저장 | P0 | 1 |

### Phase 5: Admin & Polish (Week 8)

| Task | Priority | Days |
|------|----------|------|
| Admin Dashboard UI | P1 | 2 |
| 시스템 모니터링 API | P1 | 1 |
| 썸네일 생성 서비스 | P1 | 2 |
| 테스트 + 버그 수정 | P0 | 2 |
| 문서화 | P1 | 1 |

---

## 11. Success Metrics

| Metric | Target | Measurement |
|--------|--------|-------------|
| 동시 스트림 | 20+ | Rate Limiter |
| 스트림 시작 지연 | < 2초 | API 로그 |
| SSD 캐시 히트율 | > 60% | Cache 로그 |
| 검색 응답 시간 | < 100ms | MeiliSearch |
| 서비스 가용성 | > 99% | Uptime |

---

## 12. Testing Strategy

### 12.1 테스트 피라미드

| Level | 비율 | 도구 | 범위 |
|-------|------|------|------|
| **Unit** | 70% | pytest | 개별 함수/클래스 |
| **Integration** | 20% | pytest + testcontainers | API + DB + Redis |
| **E2E** | 10% | Playwright | 전체 사용자 시나리오 |

### 12.2 Backend 테스트 (pytest)

**디렉토리 구조**:
```
api/tests/
├── unit/
│   ├── test_cache_service.py
│   ├── test_stream_service.py
│   └── test_thumbnail_service.py
├── integration/
│   ├── test_auth_api.py
│   ├── test_contents_api.py
│   └── test_stream_api.py
├── fixtures/
│   ├── conftest.py
│   ├── sample_video.mp4      # 10초, 1MB 테스트용
│   └── test_data.py
└── conftest.py
```

**주요 테스트 케이스**:
```python
# Unit Tests
class TestSSDCacheManager:
    def test_cache_hit_returns_local_path(self): ...
    def test_cache_miss_returns_nas_path(self): ...
    def test_lru_eviction_removes_oldest(self): ...

class TestStreamRateLimiter:
    def test_acquire_within_limit(self): ...
    def test_acquire_exceeds_global_limit(self): ...
    def test_acquire_exceeds_user_limit(self): ...
    def test_release_decrements_counter(self): ...

# Integration Tests
class TestAuthAPI:
    async def test_register_creates_pending_user(self): ...
    async def test_login_rejects_pending_user(self): ...
    async def test_login_accepts_active_user(self): ...

class TestStreamAPI:
    async def test_stream_returns_206_with_range(self): ...
    async def test_stream_returns_429_when_limited(self): ...
```

**Coverage Target**: 80%+

```bash
# 테스트 실행
pytest api/tests/ -v --cov=app --cov-report=html

# 단일 테스트
pytest api/tests/unit/test_cache_service.py -v

# 통합 테스트만
pytest api/tests/integration/ -v
```

### 12.3 Frontend 테스트

**도구**:
- **Vitest**: 컴포넌트 단위 테스트
- **Playwright**: E2E 시나리오 테스트

**디렉토리 구조**:
```
web/tests/
├── unit/
│   ├── VideoPlayer.test.ts
│   └── ContentCard.test.ts
├── e2e/
│   ├── auth.spec.ts
│   ├── browse.spec.ts
│   └── watch.spec.ts
└── fixtures/
```

**E2E 시나리오**:
```typescript
// e2e/auth.spec.ts
test('사용자 가입 및 승인 플로우', async ({ page }) => {
  // 1. 회원가입
  await page.goto('/register');
  await page.fill('[name="username"]', 'testuser');
  await page.click('button[type="submit"]');
  await expect(page.locator('.pending-message')).toBeVisible();

  // 2. 관리자 승인 (별도 세션)
  // 3. 로그인 성공 확인
});

// e2e/watch.spec.ts
test('비디오 재생 및 이어보기', async ({ page }) => {
  await page.goto('/watch/content-123');
  await page.waitForSelector('video');
  // Seek to 50%
  // Close and reopen
  // Verify resume from 50%
});
```

```bash
# Frontend 테스트 실행
npm run test              # Vitest
npm run test:e2e          # Playwright
```

### 12.4 테스트 데이터

| 데이터 | 용도 | 위치 |
|--------|------|------|
| sample_video.mp4 | 스트리밍 테스트 (10초, 1MB) | tests/fixtures/ |
| admin@test.com | 관리자 계정 | seed 스크립트 |
| user@test.com | 일반 사용자 (Active) | seed 스크립트 |
| pending@test.com | 대기 사용자 | seed 스크립트 |

```python
# scripts/seed_test_data.py
async def seed():
    # Admin user
    await create_user("admin", "admin@test.com", is_admin=True, status="active")
    # Active user
    await create_user("testuser", "user@test.com", status="active")
    # Pending user
    await create_user("pending", "pending@test.com", status="pending")
    # Sample content
    await create_content("Test Video", "tests/fixtures/sample_video.mp4")
```

---

## 13. Error Handling

### 13.1 글로벌 에러 응답 형식

모든 API 에러는 일관된 JSON 형식으로 반환됩니다:

```json
{
  "error": {
    "code": "AUTH_INVALID_TOKEN",
    "message": "인증 토큰이 유효하지 않습니다.",
    "details": {
      "expired_at": "2025-12-11T10:00:00Z"
    }
  }
}
```

**FastAPI Exception Handler**:
```python
# app/core/exceptions.py
class AppException(Exception):
    def __init__(self, code: str, message: str, status_code: int = 400, details: dict = None):
        self.code = code
        self.message = message
        self.status_code = status_code
        self.details = details or {}

# app/main.py
@app.exception_handler(AppException)
async def app_exception_handler(request: Request, exc: AppException):
    return JSONResponse(
        status_code=exc.status_code,
        content={
            "error": {
                "code": exc.code,
                "message": exc.message,
                "details": exc.details
            }
        }
    )
```

### 13.2 에러 코드 체계

| Prefix | 범위 | 예시 코드 |
|--------|------|-----------|
| `AUTH_` | 인증/인가 | `AUTH_INVALID_CREDENTIALS`, `AUTH_EXPIRED_TOKEN`, `AUTH_USER_PENDING` |
| `CONTENT_` | 콘텐츠 | `CONTENT_NOT_FOUND`, `CONTENT_FILE_MISSING` |
| `STREAM_` | 스트리밍 | `STREAM_RATE_LIMITED`, `STREAM_FILE_ERROR` |
| `CACHE_` | 캐시 | `CACHE_NAS_UNAVAILABLE`, `CACHE_SSD_FULL` |
| `ADMIN_` | 관리자 | `ADMIN_FORBIDDEN`, `ADMIN_USER_NOT_FOUND` |
| `VALIDATION_` | 유효성 | `VALIDATION_INVALID_EMAIL`, `VALIDATION_WEAK_PASSWORD` |

```python
# app/core/error_codes.py
class ErrorCode:
    # Auth
    AUTH_INVALID_CREDENTIALS = ("AUTH_INVALID_CREDENTIALS", "잘못된 인증 정보입니다.", 401)
    AUTH_EXPIRED_TOKEN = ("AUTH_EXPIRED_TOKEN", "토큰이 만료되었습니다.", 401)
    AUTH_USER_PENDING = ("AUTH_USER_PENDING", "승인 대기 중인 계정입니다.", 403)
    AUTH_USER_REJECTED = ("AUTH_USER_REJECTED", "거부된 계정입니다.", 403)
    AUTH_USER_SUSPENDED = ("AUTH_USER_SUSPENDED", "정지된 계정입니다.", 403)

    # Stream
    STREAM_RATE_LIMITED_GLOBAL = ("STREAM_RATE_LIMITED", "서버 동시 접속 한도 초과입니다.", 429)
    STREAM_RATE_LIMITED_USER = ("STREAM_RATE_LIMITED", "동시 재생 3개 초과입니다.", 429)

    # Content
    CONTENT_NOT_FOUND = ("CONTENT_NOT_FOUND", "콘텐츠를 찾을 수 없습니다.", 404)
```

### 13.3 HTTP 상태 코드 매핑

| Status | 사용 케이스 | 에러 코드 예시 |
|--------|------------|---------------|
| 400 | 잘못된 요청 (유효성 검증 실패) | `VALIDATION_*` |
| 401 | 인증 필요/실패 | `AUTH_INVALID_*`, `AUTH_EXPIRED_*` |
| 403 | 권한 없음 | `AUTH_USER_PENDING`, `ADMIN_FORBIDDEN` |
| 404 | 리소스 없음 | `CONTENT_NOT_FOUND` |
| 429 | Rate Limit 초과 | `STREAM_RATE_LIMITED` |
| 500 | 서버 내부 오류 | `INTERNAL_ERROR` |
| 503 | 서비스 불가 (NAS 연결 실패) | `CACHE_NAS_UNAVAILABLE` |

### 13.4 Graceful Degradation

외부 서비스 장애 시 대체 동작:

| 장애 상황 | 탐지 방법 | 대응 전략 | 사용자 영향 |
|----------|----------|----------|------------|
| **Redis 다운** | ConnectionError | DB 직접 조회 | 응답 지연 (100ms → 500ms) |
| **MeiliSearch 다운** | ConnectionError | PostgreSQL LIKE 검색 | 검색 정확도/속도 저하 |
| **NAS 연결 끊김** | SMB timeout | SSD 캐시만 서비스 | 캐시 미스 시 503 |
| **FFmpeg 실패** | Process exit code | 기본 썸네일 반환 | 일부 썸네일 미표시 |

```python
# Graceful degradation 예시
class SearchService:
    async def search(self, query: str) -> list[Content]:
        try:
            # Primary: MeiliSearch
            return await self.meilisearch.search(query)
        except MeiliSearchError:
            logger.warning("MeiliSearch unavailable, falling back to DB")
            # Fallback: PostgreSQL LIKE
            return await self.db_search(query)

    async def db_search(self, query: str) -> list[Content]:
        return await db.execute(
            select(Content).where(
                Content.title.ilike(f"%{query}%")
            ).limit(20)
        )
```

---

## 14. Logging & Monitoring

### 14.1 로그 레벨

| Level | 용도 | 예시 |
|-------|------|------|
| **DEBUG** | 개발 디버깅 | 함수 진입/종료, 변수 값 |
| **INFO** | 요청/응답, 주요 이벤트 | 스트림 시작, 로그인 성공 |
| **WARNING** | 잠재적 문제 | 캐시 미스, 재시도, 느린 쿼리 |
| **ERROR** | 복구 가능한 오류 | API 에러, 인증 실패 |
| **CRITICAL** | 시스템 중단 수준 | NAS 연결 실패, DB 다운 |

### 14.2 로그 포맷 (JSON)

```json
{
  "timestamp": "2025-12-11T10:30:00.123Z",
  "level": "INFO",
  "service": "api",
  "request_id": "req-uuid-1234",
  "user_id": "user-123",
  "action": "stream_start",
  "message": "Stream started successfully",
  "context": {
    "content_id": "content-abc",
    "cache_hit": true,
    "source": "ssd"
  },
  "duration_ms": 45
}
```

**로깅 설정**:
```python
# app/core/logging.py
import structlog

structlog.configure(
    processors=[
        structlog.processors.TimeStamper(fmt="iso"),
        structlog.processors.JSONRenderer()
    ]
)

logger = structlog.get_logger()

# 사용 예시
logger.info(
    "stream_start",
    user_id=user.id,
    content_id=content_id,
    cache_hit=True,
    source="ssd"
)
```

### 14.3 요청 추적 (Request ID)

모든 요청에 고유 ID 부여:

```python
# app/middleware/request_id.py
@app.middleware("http")
async def add_request_id(request: Request, call_next):
    request_id = request.headers.get("X-Request-ID", str(uuid.uuid4()))
    request.state.request_id = request_id

    response = await call_next(request)
    response.headers["X-Request-ID"] = request_id
    return response
```

### 14.4 메트릭 수집

| Metric | 타입 | 수집 방법 | 알림 임계값 |
|--------|------|----------|------------|
| `streams_active` | Gauge | Redis `stream:active` | > 18 (90%) |
| `streams_total` | Counter | 요청 카운트 | - |
| `cache_hit_rate` | Gauge | L2 캐시 로그 집계 | < 40% |
| `api_latency_p99` | Histogram | 미들웨어 측정 | > 500ms |
| `error_rate` | Gauge | 에러 로그 / 전체 요청 | > 5% |
| `ssd_cache_usage` | Gauge | 디스크 사용량 | > 90% |

```python
# 메트릭 미들웨어
@app.middleware("http")
async def metrics_middleware(request: Request, call_next):
    start = time.perf_counter()
    response = await call_next(request)
    duration = (time.perf_counter() - start) * 1000

    logger.info(
        "request_completed",
        method=request.method,
        path=request.url.path,
        status=response.status_code,
        duration_ms=duration
    )
    return response
```

### 14.5 알림 조건

| 조건 | 임계값 | 알림 방법 | 우선순위 |
|------|--------|----------|---------|
| NAS 연결 실패 | 1회 | 즉시 알림 | P0 |
| 에러율 급증 | 5분간 10%+ | 즉시 알림 | P0 |
| 동시 스트림 한도 | 18/20 (90%) | 경고 | P1 |
| SSD 캐시 용량 | 90%+ | 경고 | P1 |
| API 응답 지연 | P99 > 1초 | 경고 | P2 |

### 14.6 로그 저장 위치

```
D:\WSOPTV\logs\
├── api\
│   ├── api.log           # 현재 로그
│   ├── api.log.1         # 로테이션 (1일 보관)
│   └── ...
├── stream\
│   └── stream.log        # 스트리밍 전용 로그
└── error\
    └── error.log         # 에러 전용 (30일 보관)
```

---

## 15. Backup Strategy

### 15.1 백업 대상

| 대상 | 주기 | 보관 기간 | 저장 위치 | 방법 |
|------|------|----------|----------|------|
| **PostgreSQL** | 일 1회 (03:00) | 30일 | `Z:\BACKUP\db\` | pg_dump + gzip |
| **Redis RDB** | 시간 1회 | 7일 | `Z:\BACKUP\redis\` | BGSAVE + 복사 |
| **설정 파일** | 변경 시 | Git 기록 | GitHub | .env 제외 |
| **MeiliSearch 인덱스** | 주 1회 | 4주 | `Z:\BACKUP\meili\` | 스냅샷 |

### 15.2 백업 스크립트

```powershell
# scripts/backup.ps1
param(
    [string]$Type = "all"  # all, db, redis, meili
)

$date = Get-Date -Format "yyyyMMdd-HHmmss"
$backupRoot = "Z:\BACKUP"

function Backup-Database {
    Write-Host "Backing up PostgreSQL..." -ForegroundColor Cyan
    $output = "$backupRoot\db\wsoptv-$date.sql.gz"

    $env:PGPASSWORD = $env:DB_PASSWORD
    pg_dump -h localhost -U wsoptv wsoptv | gzip > $output

    Write-Host "Database backup: $output" -ForegroundColor Green

    # 30일 이상 된 백업 삭제
    Get-ChildItem "$backupRoot\db\*.gz" |
        Where-Object { $_.LastWriteTime -lt (Get-Date).AddDays(-30) } |
        Remove-Item
}

function Backup-Redis {
    Write-Host "Backing up Redis..." -ForegroundColor Cyan

    # BGSAVE 실행
    redis-cli BGSAVE
    Start-Sleep -Seconds 5

    $output = "$backupRoot\redis\dump-$date.rdb"
    Copy-Item "D:\WSOPTV\redis\dump.rdb" $output

    Write-Host "Redis backup: $output" -ForegroundColor Green

    # 7일 이상 된 백업 삭제
    Get-ChildItem "$backupRoot\redis\*.rdb" |
        Where-Object { $_.LastWriteTime -lt (Get-Date).AddDays(-7) } |
        Remove-Item
}

function Backup-MeiliSearch {
    Write-Host "Backing up MeiliSearch..." -ForegroundColor Cyan
    $output = "$backupRoot\meili\meili-$date"

    # MeiliSearch 스냅샷 생성
    Invoke-RestMethod -Uri "http://localhost:7700/snapshots" -Method Post
    Start-Sleep -Seconds 10

    Copy-Item "D:\WSOPTV\meili\snapshots\*" $output -Recurse

    Write-Host "MeiliSearch backup: $output" -ForegroundColor Green
}

switch ($Type) {
    "db"    { Backup-Database }
    "redis" { Backup-Redis }
    "meili" { Backup-MeiliSearch }
    "all"   {
        Backup-Database
        Backup-Redis
        Backup-MeiliSearch
    }
}

Write-Host "`nBackup completed!" -ForegroundColor Green
```

### 15.3 자동 백업 스케줄

```powershell
# Windows Task Scheduler 등록
# scripts/setup-backup-schedule.ps1

$action = New-ScheduledTaskAction -Execute "powershell.exe" `
    -Argument "-File D:\WSOPTV\scripts\backup.ps1 -Type all"

$trigger = New-ScheduledTaskTrigger -Daily -At "03:00"

Register-ScheduledTask -TaskName "WSOPTV-DailyBackup" `
    -Action $action -Trigger $trigger `
    -Description "WSOPTV 일일 백업"
```

### 15.4 복구 절차

#### PostgreSQL 복구
```powershell
# 1. 서비스 중지
Stop-Service postgresql-x64-16

# 2. 최신 백업 확인
$latest = Get-ChildItem "Z:\BACKUP\db\*.gz" | Sort-Object LastWriteTime -Descending | Select-Object -First 1

# 3. 복원
$env:PGPASSWORD = $env:DB_PASSWORD
gunzip -c $latest.FullName | psql -h localhost -U wsoptv -d wsoptv

# 4. 서비스 시작
Start-Service postgresql-x64-16
```

#### Redis 복구
```powershell
# 1. Redis 중지
Stop-Process -Name redis-server

# 2. 최신 백업 복사
$latest = Get-ChildItem "Z:\BACKUP\redis\*.rdb" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
Copy-Item $latest.FullName "D:\WSOPTV\redis\dump.rdb"

# 3. Redis 시작
Start-Process redis-server -WindowStyle Hidden
```

#### SSD 캐시
```
캐시는 복구 불필요 - 요청 시 자동 재생성됨
```

### 15.5 DR (Disaster Recovery)

| 시나리오 | RTO (복구 시간) | RPO (데이터 손실) | 복구 절차 |
|----------|----------------|------------------|----------|
| **DB 손상** | 1시간 | 최대 24시간 | 최신 백업에서 복원 |
| **Redis 손상** | 15분 | 최대 1시간 | RDB 파일 복원 |
| **서버 장애** | 4시간 | 최대 24시간 | 새 서버 프로비저닝 + 백업 복원 |
| **SSD 장애** | 30분 | 0 (캐시만) | SSD 교체 + 캐시 재생성 |
| **NAS 장애** | - | 콘텐츠 복구 불가 | NAS 복구 대기 (별도 백업 필요) |

### 15.6 백업 검증

월 1회 복구 테스트 수행:

```powershell
# scripts/verify-backup.ps1
# 테스트 환경에서 최신 백업 복원 후 무결성 검증

# 1. 테스트 DB에 복원
$latest = Get-ChildItem "Z:\BACKUP\db\*.gz" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
gunzip -c $latest.FullName | psql -h localhost -U wsoptv -d wsoptv_test

# 2. 레코드 수 확인
psql -h localhost -U wsoptv -d wsoptv_test -c "SELECT COUNT(*) FROM users;"
psql -h localhost -U wsoptv -d wsoptv_test -c "SELECT COUNT(*) FROM contents;"

# 3. 결과 로깅
Write-Host "Backup verification completed: $latest" -ForegroundColor Green
```

---

## 16. Block Architecture & Agent Separation

### 16.1 문제 해결 목표

```
[기존 문제]
Claude 1개 → 전체 코드베이스 → 컨텍스트 폭발 → 망각 → 코드 오염

[해결책]
┌─────────────────────────────────────────────────────────────┐
│                    Orchestration Layer                       │
│                   (컨텍스트 최소화)                          │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐       │
│  │  Auth   │  │ Content │  │ Stream  │  │  Cache  │       │
│  │  Block  │  │  Block  │  │  Block  │  │  Block  │       │
│  │         │  │         │  │         │  │         │       │
│  │ Agent A │  │ Agent B │  │ Agent C │  │ Agent D │       │
│  │ PRD-A   │  │ PRD-B   │  │ PRD-C   │  │ PRD-D   │       │
│  └─────────┘  └─────────┘  └─────────┘  └─────────┘       │
│       │            │            │            │              │
│       └────────────┴─────┬──────┴────────────┘              │
│                          │                                   │
│                    [Message Bus]                             │
│                                                              │
└─────────────────────────────────────────────────────────────┘

→ 각 에이전트는 자기 블럭만 집중 → 컨텍스트 유지 → 오염 방지
```

### 16.2 Block List

| Block ID | 이름 | 책임 | 전담 에이전트 | PRD |
|----------|------|------|---------------|-----|
| `auth` | Auth Block | 인증, 인가, 사용자 관리 | `auth-agent` | `docs/blocks/01-auth.md` |
| `content` | Content Block | 콘텐츠 CRUD, 카탈로그 | `content-agent` | `docs/blocks/02-content.md` |
| `stream` | Stream Block | 비디오 스트리밍 | `stream-agent` | `docs/blocks/03-stream.md` |
| `cache` | Cache Block | 4-Tier 캐시 관리 | `cache-agent` | `docs/blocks/04-cache.md` |
| `admin` | Admin Block | 관리자 대시보드 | `admin-agent` | `docs/blocks/05-admin.md` |
| `search` | Search Block | MeiliSearch 검색 | `search-agent` | `docs/blocks/06-search.md` |
| `worker` | Worker Block | 백그라운드 작업 | `worker-agent` | `docs/blocks/07-worker.md` |

### 16.3 핵심 원칙

| 원칙 | 설명 |
|------|------|
| **블럭 독립성** | 각 블럭은 자체 PRD, 테스트, 에이전트를 가짐 |
| **중앙 조율** | 오케스트레이션만 블럭 간 통신 중재 |
| **계약 기반** | 블럭 간 인터페이스는 명시적 계약으로 정의 |
| **장애 격리** | 한 블럭 실패가 다른 블럭에 전파되지 않음 |

### 16.4 에이전트 규칙

각 블럭 전담 에이전트는 다음 규칙을 준수:

```markdown
## [Block] 전담 에이전트 규칙

1. **컨텍스트 제한**
   - `api/app/blocks/[block]/` 디렉토리만 수정
   - 다른 블럭 코드 읽기 금지 (인터페이스만 참조)

2. **수정 가능 파일**
   - `api/app/blocks/[block]/**/*`
   - `tests/blocks/[block]/**/*`
   - `docs/blocks/[block].md`

3. **수정 불가 파일**
   - `api/app/blocks/*/` (다른 블럭)
   - `api/app/orchestration/` (오케스트레이터)

4. **통신 규칙**
   - 다른 블럭 호출 시 반드시 오케스트레이터 경유
   - 직접 import 금지
```

### 16.5 블럭 간 통신

```python
# ❌ 금지: 직접 import
from api.app.blocks.auth.service import AuthService

# ✅ 권장: 오케스트레이터 경유
user = await orchestrator.request(
    target="auth",
    action="validate_token",
    payload={"token": token}
)
```

### 16.6 Directory Structure

```
api/app/
├── orchestration/           # 오케스트레이션 레이어
│   ├── orchestrator.py      # 메인 오케스트레이터
│   ├── message_bus.py       # Redis Pub/Sub
│   ├── registry.py          # 블럭 레지스트리
│   └── contracts/           # 블럭 간 계약
│
├── blocks/                  # 블럭들 (독립 단위)
│   ├── auth/
│   ├── content/
│   ├── stream/
│   ├── cache/
│   ├── admin/
│   ├── search/
│   └── worker/
│
└── main.py                  # 블럭 등록

docs/blocks/
├── 00-orchestration.md     # 오케스트레이션 PRD
├── 01-auth.md              # Auth Block PRD
├── 02-content.md           # Content Block PRD
├── 03-stream.md            # Stream Block PRD
├── 04-cache.md             # Cache Block PRD
├── 05-admin.md             # Admin Block PRD
├── 06-search.md            # Search Block PRD
└── 07-worker.md            # Worker Block PRD
```

### 16.7 Success Metrics

| Metric | Target |
|--------|--------|
| 블럭 독립성 | 블럭 간 직접 import 0 |
| 에이전트 컨텍스트 | 블럭당 < 500줄 |
| 장애 격리 | 한 블럭 장애 시 다른 블럭 정상 |
| 테스트 독립성 | Mock 없이 다른 블럭 호출 0 |

---

## Appendix

### A. NAS 파일 구조 예시

```
Z:\ARCHIVE\
├── WSOP\
│   ├── 2024\
│   │   ├── Main Event\
│   │   │   ├── Day 1.mp4
│   │   │   ├── Day 2.mp4
│   │   │   └── Final Table.mp4
│   │   └── High Roller\
│   └── 2023\
├── HCL\
│   ├── Season 1\
│   └── Season 2\
├── PAD\
└── GGMillions\
```

### B. Content Import Script

```python
# scripts/import_contents.py
async def scan_and_import(nas_root: Path):
    """NAS 폴더 스캔 후 DB에 등록"""
    for catalog_dir in nas_root.iterdir():
        if not catalog_dir.is_dir():
            continue

        catalog = await get_or_create_catalog(catalog_dir.name)

        for video_file in catalog_dir.rglob("*.mp4"):
            await create_content(
                catalog_id=catalog.id,
                title=video_file.stem,
                nas_path=str(video_file),
                duration=await get_duration(video_file),
                file_size=video_file.stat().st_size
            )
```

---

**Document History**

| Version | Date | Author | Changes |
|---------|------|--------|---------|
| 1.0.0 | 2025-12-11 | Claude Code | Initial draft |
| 1.1.0 | 2025-12-11 | Claude Code | Testing Strategy, Error Handling, Logging & Monitoring, Backup Strategy 섹션 추가 |
| 1.2.0 | 2025-12-11 | Claude Code | Block Architecture & Agent Separation 섹션 추가 (Section 16) |
| 1.3.0 | 2025-12-11 | Claude Code | 초기 관리자 계정 (garimto/1234) 및 회원가입 플로우 상세 명세 추가 (Section 5.1) |
| 1.4.0 | 2025-12-11 | Claude Code | Netflix 스타일 목업 디자인 추가 - Hero Billboard, Row 레이아웃, Hover 효과 (Section 5.2) |
